...
3. Pass the `$ARGUMENTS` above down as the `{user_message}` captured by Bootstrap. Bootstrap runs, from the directory where start.aisop.json itself lives, `python -B -X utf8 ${CLAUDE_SKILL_DIR}\soulagent\start.py "<user_message>"` using an argument-list or equivalent safe invocation; do not concatenate user text into a shell command string. `start.py` self-resolves engine_dir / aiap_dir via `__file__`, pointing at the engine and registry inside soulagent/.
...
- **PYTHON_TOOLS**: when calling `agent_update_index.py` / `dispatch_audit.py` / `user_gate_audit.py` etc., pass JSON via **STDIN, temp JSON files + file-input flags, or an args-list**; **never** inline JSON literals in PowerShell; and **use `python -B -X utf8`** for cache/bootstrap tools (on Windows, JSON containing non-ASCII / emoji raises a surrogate error without UTF-8 mode, and `-B` prevents pycache residue).
